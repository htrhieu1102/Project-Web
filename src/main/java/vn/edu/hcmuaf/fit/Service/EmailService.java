...
public class EmailService {
    ...
    public void OTPRegister(OTPRegister otpRegister) {
        Properties properties = new Properties();
        properties.put("mail.smtp.auth", "true");
        properties.put("mail.smtp.starttls.enable", "true");
        properties.put("mail.smtp.host", "smtp.gmail.com");
        properties.put("mail.smtp.port", "587");
        properties.put("mail.smtp.name", "Hạc Key");
        String subject = "Đăng ký tài khoản Hạc Key Shop";
        String text = "<h1 style=\"padding: 0; font-size: 25px;color: #2dd100;font-family:sans-serif\">Mã OTP đăng ký tài khoản Hạc Key Shop</h1>" +
                "<p style=\"padding: 0;font-size: 14px;color: #000000;font-family:sans-serif\">Mã OTP: <strong style=\"font-size: 18px; color: #2dd100; font-family:sans-serif\">" + otpRegister.getCode() + "</strong></p>" +
                "<p style=\"padding: 0;font-size: 14px;color: #000000;font-family:sans-serif\">Cảm ơn bạn,</p><p style=\"padding: 0;font-size: 14px;color: #2dd100;font-family:sans-serif\">Hạc Key shop.</p>";
        ...
        try {
            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress(account));
            message.setRecipient(Message.RecipientType.TO, new InternetAddress(otpRegister.getUser().getEmail()));
            message.setHeader("Content-Type", "text/html; charset=UTF-8");
            message.setSubject(subject);
            message.setContent(text, "text/html; charset=UTF-8");
            Transport.send(message);
